...
kable(TB) %>% kable_styling(bootstrap_options = "striped", full_width = FALSE, position = "left")
```
...
kable(RMAPres) %>% kable_styling(bootstrap_options = "striped", full_width = FALSE, position = "left")
...
```{r BetaTestOther2, echo=FALSE, fig.height=10, fig.width=10,  message=FALSE}
#library(pairwiseAdonis) # Pairwise comprisons were done using this package but, was done in a most recent R version 4.3.1
for(i in 1:5){
OTU_Plot <- transform_sample_counts(Vec2[[i]], function(x) 100 * x/sum(x))
OTU_Plot <- transform_sample_counts(OTU_Plot, function(x) log(1 + x) )
OTU_Plot2 <- subset_samples(OTU_Plot, BMI!="NA") 
...
#Where n is the number of samples in the comparison and k is the number of predictors
#print(Adonis)
Interaction <- (round(Adonis$`Pr(>F)`[6], 4))
...
#pair.wise function ran in R version 4.3.1
...
No significant differences were detected in this analysis which was performed in R version 4.3.1 because the pair.wise function doesn't work in the current R version.
...
```{r DAHLA, echo=FALSE, results='asis'}
  for(i in 1:5){
  VecOF <- prune_samples(sample_sums(VecO[[i]]) > 5000, VecO[[i]])
  X <- cut(sample_data(VecOF)$BMI, breaks = c(-Inf,18.5,25,30,Inf), labels = c("Underweight","Normal","Overweight", "Obese"))
  sample_data(VecOF)$BMI_Cat <- X
  ...
  if(length(DA_GG)>0){
    if(k==1){
     ...
     #cat('\n####', Tax[i], "in", Dummy[k], '\n')
     print(kable(Results) %>% kable_styling(bootstrap_options = "striped", full_width = FALSE, position = "left"))
    }
    if(k==2){
...
#This plot was done using the R package microbiomeSeq in R version 4.3.1
...
```{r T1Ddist, fig.height=7, fig.width=9, echo=FALSE, results='asis'}
...
print(kable(X) %>% kable_styling(bootstrap_options = "striped", full_width = FALSE, position = "left"))
```
...
```{r CSTdist, echo=FALSE, fig.height=4, fig.width=5, message=FALSE}
#, results='asis'
...
#Done using R version 4.3.1 package microbiomeSeq
...
```{r CSTdistP, fig.height=5, fig.width=8, echo=FALSE}
...
#Plot was done using R version 4.3.1 and R package microbiomeSeq
...
```{r CSTdistLacto, fig.height=7, fig.width=9, echo=FALSE}
for(k in 2:4){
CSTContM$T1DsParity <- paste(CSTContM$T1Dstatus, CSTContM$Parity, sep=".")
X <- which( colnames(CSTContM)=="T1DsParity" )
...
```{r CSTdistLactoBMI, fig.height=7, fig.width=9, echo=FALSE}
for(k in 2:4){
CSTContM <- subset(CSTContM, BMI_Cat!="NA")
CSTContM$T1DsBMI <- paste(CSTContM$T1Dstatus, CSTContM$BMI_Cat, sep=".")
X <- which( colnames(CSTContM)=="T1DsBMI" )
...
```{r LogisticLacto, fig.height=7, fig.width=5, echo=FALSE, results='asis'}
...
```{r AdonisMyco, echo=FALSE, fig.height=10, fig.width=10}
...
kable(AdonisR) %>% kable_styling(bootstrap_options = "striped", full_width = FALSE, position = "left")
R <- AdonisR$`Pr(>F)`[3]
## 95% CI
...
```{r BetaTestOther2My, echo=FALSE, fig.height=10, fig.width=10, message=FALSE}
for(i in 1:5){
...
#if(i==1){
kable(Adonis) %>% kable_styling(bootstrap_options = "striped", full_width = FALSE, position = "left")
#95% CI
CI.Rsq(0.011, n=133, k=5)
Int <- (round(Adonis$`Pr(>F)`[6], 4))

}
```


##Differential abundance (T1D status)


```{r DAMyco, echo=FALSE, fig.height=10, fig.width=10, message=FALSE}
for(i in 1:5){
VecOFilt <- Vec3[[i]]
...
```{r DAHLAMyc, echo=FALSE, results='asis'}
for(i in 1:5){
  VecOF <- Vec3[[i]]
  X <- cut(sample_data(VecOF)$BMI, breaks = c(-Inf,18.5,25,30,Inf), labels = c("Underweight","Normal","Overweight", "Obese"))
  sample_data(VecOF)$BMI_Cat <- X
...
###Composed figures (T1D status for manuscript)


```{r Composed_F2, echo=FALSE, fig.height=10, fig.width=10, message=FALSE}
