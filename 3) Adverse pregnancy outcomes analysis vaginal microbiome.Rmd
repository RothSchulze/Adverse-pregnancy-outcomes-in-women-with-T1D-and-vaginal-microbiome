...
```{r AlphaTest, echo=FALSE, fig.height=5, fig.width=10, results='asis'}
...
marginal = emmeans(in_GAD, ~ BMIGAD)
...
```{r BetaTest, echo=FALSE, fig.height=10, fig.width=10, results='asis'}
...
```{r AlphaTest, echo=FALSE, fig.height=10, fig.width=10, results='asis'}
...
##Genitourinary tract infection (GUTI)
...
for(i in 1:5){
Vec2[[i]] <- phyloseq(otu_table(Vec2[[i]], taxa_are_rows = F), sample_data(NewMet), tax_table(Vec2[[i]]), phy_tree(Vec2[[i]]))
Vec2[[i]] <- subset_samples(Vec2[[i]], GU.tract.infection!="NA")
}
for(i in 1:5){
VecMy[[i]] <- phyloseq(otu_table(VecMy[[i]], taxa_are_rows = F), sample_data(NewMet), tax_table(VecMy[[i]]), phy_tree(VecMy[[i]]))
VecMy[[i]] <- subset_samples(VecMy[[i]], GU.tract.infection!="NA")

}
for(i in 1:5){
VecO[[i]] <- phyloseq(otu_table(VecO[[i]], taxa_are_rows = F), sample_data(NewMet), tax_table(VecO[[i]]), phy_tree(VecO[[i]]))
VecO[[i]] <- subset_samples(VecO[[i]], GU.tract.infection!="NA")

}
TB <- table(sample_data(Vec2[[1]])$GU.tract.infection, sample_data(Vec2[[1]])$T1Dstatus)
```

For analysing the vaginal microbiome of women who had or did not have genitourinary tract infections [GUTI] there are a total of 284 samples for which we have the GUTI status. The samples are distributed between women with and without T1D as follows:
...
0: women without GUTI and 1: women with GUTI
...
DivCal_R <- estimate_richness(OTUVag_RA, measures=c("Observed", "InvSimpson"))
DivCal_R_df <- data.frame(DivCal_R, sample_data(OTUVag_RA))
GUTIcol <- c("deepskyblue2", "darkmagenta")
```

Pale blue: No GUTI; Magenta: GUTI

```{r AlphaTest, echo=FALSE, fig.height=5, fig.width=10, results='asis'}
Dummy <- OTUVag_RA
sample_data(Dummy)$GUTI <- as.factor(sample_data(Dummy)$GU.tract.infection)
DivCal_R <- estimate_richness(Dummy, measures=c("Observed", "InvSimpson"))
DivCal_R_df <- data.frame(DivCal_R, sample_data(Dummy))
DivCal_R_df$SeqRun <- factor(DivCal_R_df$SeqRun)
DivCal_R_df$Parity <- factor(DivCal_R_df$Parity)
DivCal_R_df$motherid <- factor(DivCal_R_df$motherid)
DivCal_R_df$GUTI <- factor(DivCal_R_df$GUTI)
#CATEGORIZE BMI FOR TEST
DivCal_R_df$BMI_Cat <- cut(DivCal_R_df$BMI, breaks = c(-Inf,18.5,25,30,Inf), labels = c("Underweight","Normal","Overweight", "Obese"))
DivCal_R_df$BMI_Cat <- as.factor(DivCal_R_df$BMI_Cat)
DivCal_R_df <- subset(DivCal_R_df, BMI_Cat!="NA")

ob_GUTI <- glmer.nb(Observed ~  BMI_Cat*GUTI + Parity*GUTI + T1Dstatus*GUTI + (1|motherid) + (1|SeqRun), data=DivCal_R_df) 
ResObs <- Anova(ob_GUTI)
RGUTI <- round(ResObs$`Pr(>Chisq)`[2], 3)

Rich_P <- plot_richness(Dummy, x="GUTI", scales = "fixed", measures=c("Observed"))
Rich_P$layers <- Rich_P$layers[-1]
Rich_Plot1 <- Rich_P + geom_boxplot(data=Rich_P$data, aes(color= GUTI), alpha=0.1, size=1)  +  theme(axis.title.y = element_text(size=14), axis.title.x = element_blank()) + theme(axis.text.y =element_text(size=15), axis.text.x=element_blank(), plot.title = element_text(size=15))  + theme(legend.title = element_text(size=15), legend.text = element_text(size=15)) + labs(color="GUTI") + geom_jitter()  +  annotate("text", label = paste("P=", RGUTI), x=1.5, y =40, size = 4, colour = "black") + theme(legend.position='none') + scale_fill_manual(values=GUTIcol) + scale_color_manual(values=GUTIcol)

# InvSimpson
in_GUTI <- glmer.nb(round(((DivCal_R_df$InvSimpson)*1000),0) ~ BMI_Cat*GUTI + Parity*GUTI  + T1Dstatus*GUTI + (1|motherid) + (1|SeqRun), data=DivCal_R_df)
ResObs3 <- Anova(in_GUTI)
IRGUTI <- round(ResObs3$`Pr(>Chisq)`[2], 3)

Rich_P <- plot_richness(Dummy, x="GUTI", scales = "fixed", measures=c("InvSimpson"))
Rich_P$layers <- Rich_P$layers[-1]
Rich_Plot2 <- Rich_P + geom_boxplot(data=Rich_P$data, aes(color= GUTI), alpha=0.1, size=1)  +  theme(axis.title.y = element_text(size=14), axis.title.x = element_blank()) + theme(axis.text.y =element_text(size=15), axis.text.x=element_blank(), plot.title = element_text(size=15))  + theme(legend.title = element_text(size=15), legend.text = element_text(size=15)) + labs(color="GUTI") + geom_jitter() +  annotate("text", label = paste("P=", IRGUTI), x=1.5, y =8, size = 4, colour = "black")   + scale_fill_manual(values=GUTIcol) + scale_color_manual(values=GUTIcol) + theme(legend.position='none')
...
OTU_Plot <- transform_sample_counts(Vec2[[i]], function(x) 100 * x/sum(x))
OTU_Plot <- transform_sample_counts(OTU_Plot, function(x) log(1 + x) )
sample_data(OTU_Plot)$GUTI <- as.factor(sample_data(OTU_Plot)$GU.tract.infection)
OTU_Plot2 <- subset_samples(OTU_Plot, BMI!="NA")
...
Meta_df2$SeqRun <- as.factor(Meta_df2$SeqRun)
Meta_df2$Parity <- as.factor(Meta_df2$Parity)
Meta_df2$GUTI <- as.factor(Meta_df2$GUTI)
Meta_df2$BMI <- as.numeric(Meta_df2$BMI)
Meta_df2$motherid <- as.factor(Meta_df2$motherid)
Meta_df2$T1Dstatus <- as.factor(Meta_df2$T1Dstatus)

#testing interaction between T1Dstatus, parity and GUTI with GUTI  --> not significant
AdonisR <- adonis2(D2 ~  SeqRun + Parity*GUTI + BMI*GUTI + T1Dstatus*GUTI, strata=Meta_df2$motherid, data=Meta_df2) 
print(AdonisR)
A2Res <- round(AdonisR$`Pr(>F)`[3], 2)

sample_data(OTU_Plot2)$GUTI <- as.factor(sample_data(OTU_Plot2)$GUTI)
ordination_OTU<-ordinate(OTU_Plot2, method="CAP", distance="bray", ~  GUTI)
p_OTU12<-plot_ordination(OTU_Plot2, ordination_OTU, type="samples",  color="GUTI")
PlotList[[i]] <- p_OTU12 + geom_point(size = 3)  + theme(axis.title.y = element_text(size=14), axis.title.x = element_text(size=14)) + theme(axis.text.y =element_text(size=15), axis.text.x=element_text(size=15), plot.title = element_text(size=15))  + theme(legend.title = element_text(size=15), legend.text = element_text(size=15))  +  annotate("text", label = paste("P=", IRGUTI), x=1.5, y =1.5, size = 4, colour = "black")  + theme(legend.position='none')  + scale_fill_manual(values=GUTIcol) + scale_color_manual(values=GUTIcol) 
}
...
```{r DAGUTIMicro, echo=FALSE}
for(i in 1:5){
  VecOFilt <- VecO[[i]]
  VecOFilt <- prune_samples(sample_sums(VecOFilt) > 5000, VecOFilt)
  sample_data(VecOFilt)$GUTI <- as.factor(sample_data(VecOFilt)$GU.tract.infection)
  Counts <- as.data.frame(t(otu_table(VecOFilt)))
  # Metadata
  Meta_df <- as.data.frame(as.matrix(sample_data(VecOFilt)))
  Meta_df$SeqRun <- as.factor(Meta_df$SeqRun)
  ...
  Meta_df$motherid <- as.factor(as.character(Meta_df$motherid))
  contrasts(Meta_df$motherid) <- contr.sum(levels(Meta_df$motherid))
  Meta_df$GUTI <- as.factor(Meta_df$GUTI)
  contrasts(Meta_df$GUTI) <- contr.sum(levels(Meta_df$GUTI))
  ...
  Meta_df$GUTIT1D <-as.factor(paste(Meta_df$GUTI, Meta_df$T1Dstatus, sep=".")) #Composed factor to test differences due to GUTI within T1D status levels
  
  design <- model.matrix(~0 + GUTIT1D + Parity + SeqRun, data=Meta_df)
  
  dge <- DGEList(Counts, group= Meta_df$GUTIT1D)
  
  #Prevalence filter
  M <- dge$counts
  M[M>0] <-1
  keepP <- rowSums(M) > 140 #140 is about 50% of the samples (284) 
  dge <- dge[keepP,,keep.lib.sizes=FALSE]
  dge <- dge[,which(!dge$samples$lib.size == 0)]
  # Metadata
  dgeN <- colnames(dge$counts)
  ...
  levels(Meta_df$T1Dstatus) <- c("NonT1D", "T1D")
  contrasts(Meta_df$T1Dstatus) <- contr.sum(levels(Meta_df$T1Dstatus))
  Meta_df$GUTIT1D <-as.factor(paste(Meta_df$GUTI, Meta_df$T1Dstatus, sep="."))
  design <- model.matrix(~0 + GUTIT1D + Parity + SeqRun, data=Meta_df)
  ...
  ## Contrast GUTI_levels in general and within T1D status
  cont <- makeContrasts(GUTI=((GUTIT1D0.NonT1D + GUTIT1D0.T1D)/2 - (GUTIT1D1.NonT1D + GUTIT1D1.T1D)/2), GUTIT1D= GUTIT1D0.T1D - GUTIT1D1.T1D, GUTINT1D= GUTIT1D0.NonT1D - GUTIT1D1.NonT1D, levels=design)
  fitc <- contrasts.fit(fit, contrasts = cont)
  fitc <- eBayes(fitc, robust=TRUE)
  DT<- decideTests(fitc)
  summary(DT)
  
  #Results top table for GUTI in general
  TT <- topTable(fitc, coef = "GUTI", n=Inf)
  TO <- TT
  TV<- vector()
  Mean <- cbind(data.frame(rowMeans(fit$coef[rownames(TO),1:2])), data.frame(rowMeans(fit$coef[rownames(TO),3:4])))
  colnames(Mean)[1:2] <- c("MEAN:No GUTI", "MEAN:GUTI")
  SE <- fit$stdev.unscaled * sqrt(fitc$s2.post)
  SE <- cbind(data.frame(rowMeans(SE[rownames(TO),1:2])), data.frame(rowMeans(SE[rownames(TO),3:4])))
  colnames(SE)[1:2] <- c("SE:No GUTI", "SE:GUTI")
  MeanSE <- cbind(TT[,c(1,4:5)], Mean, SE)
  MeanSE <- MeanSE[(MeanSE$adj.P.Val<0.1),]
  ...
  if(nrow(MeanSE)>0){
    ...
    Y <- data.frame(str_split_fixed(X$variable, ":", 2))
    Z <- cbind(X[1], Y, X[3])
    Z$X2[1:length(MeanSE$adj.P.Val)] <- rep("GUTI", length(MeanSE$adj.P.Val))
    Z2 <- dcast(Z, Taxa + X2 ~ X1)
    Z2$adj.P.Val <- round(Z2$adj.P.Val, 2)
    colnames(Z2)[2] <- "GUTI status"
    if(i==1){
    Zspe <- Z2
    }
    if(i==2){
...
#Results from DA within T1D and non-T1D women (No significant differences in specific and different taxa (from the ones detected in all women) due to GUTI were found when samples within T1D status were analysed)
##Women with T1D  
TT <- topTable(fitc, coef = "GUTIT1D", n=Inf)
##Women without T1D
TT <- topTable(fitc, coef = "GUTINT1D", n=Inf)
}
  #For plot
    ZA <- rbind(Zspe, Zgen, Zor)
    ZA$Taxa <- as.factor(ZA$Taxa)
    ZA$Taxa <- factor(ZA$Taxa, levels=ZA$Taxa[duplicated(ZA$Taxa)])
    ZA$`GUTI status` <- relevel(ZA$`GUTI status`, ref="No GUTI")
  #Plotting
    pd <- position_dodge(0.5)
    p2 <- ggplot(ZA, aes(x=Taxa, y=MEAN, colour=`GUTI status`, label=adj.P.Val))
    DAMicroGUTI <-  p2 + geom_errorbar(aes(ymin=MEAN-SE, ymax=MEAN+SE), size=1.5, width=.2, position=pd) + geom_point(position=pd, size=3, shape=21, fill="white") + theme(axis.title.x = element_blank()) + geom_text(angle = 0, vjust=1, hjust=1, color="black", size=4, check_overlap = TRUE)+ theme(axis.text.y =element_text(size=14), axis.text.x = element_text(angle = 45, hjust=1, size=12, face="italic")) + theme(legend.title = element_text(size=13, face="bold")) + scale_fill_manual(values=GUTIcol) + scale_color_manual(values=GUTIcol)   + theme(plot.title = element_text(size=12)) + scale_y_continuous(labels = scales::number_format(accuracy = 0.1))  + ylab("log2-transformed fitted values") + theme(legend.position='none') 
    plot(DAMicroGUTI)
```

##Group B Streptococcus infection (GBSI)
...
for(i in 1:5){
Vec2[[i]] <- phyloseq(otu_table(Vec2[[i]], taxa_are_rows = F), sample_data(NewMet), tax_table(Vec2[[i]]), phy_tree(Vec2[[i]]))
Vec2[[i]] <- subset_samples(Vec2[[i]], GBS!="NA")
}
for(i in 1:5){
VecMy[[i]] <- phyloseq(otu_table(VecMy[[i]], taxa_are_rows = F), sample_data(NewMet), tax_table(VecMy[[i]]), phy_tree(VecMy[[i]]))
VecMy[[i]] <- subset_samples(VecMy[[i]], GBS!="NA")

}
for(i in 1:5){
VecO[[i]] <- phyloseq(otu_table(VecO[[i]], taxa_are_rows = F), sample_data(NewMet), tax_table(VecO[[i]]), phy_tree(VecO[[i]]))
VecO[[i]] <- subset_samples(VecO[[i]], GBS!="NA")
}
T1Dcol <- c("red", "blue")

TB <- table(sample_data(Vec2[[1]])$GBS, sample_data(Vec2[[1]])$T1Dstatus)
```

For analysing the vaginal microbiome of women who had or did not have Group B streptococcus [GBS] there are a total of 187 samples for which we have the GBS status. The samples are distributed between women with and without T1D as follows:
...
0: women without GBS and 1: women with GBS

###Alpha diversity

```{r, AlphaGBSImi, echo=FALSE}
OTUVag_RA <- Vec2[[1]]
DivCal_R <- estimate_richness(OTUVag_RA, measures=c("Observed", "InvSimpson"))
DivCal_R_df <- data.frame(DivCal_R, sample_data(OTUVag_RA))
GBScol <- c("cornflowerblue", "darkgoldenrod1")
```

Pale blue: No GBS; Gold: GBS.

```{r AlphaTestGBSImi, echo=FALSE, fig.height=5, fig.width=10, results='asis'}
sample_data(OTUVag_RA)$GBS <- as.factor(sample_data(OTUVag_RA)$GBS)
Dummy <- OTUVag_RA
DivCal_R <- estimate_richness(Dummy, measures=c("Observed", "InvSimpson"))
DivCal_R_df <- data.frame(DivCal_R, sample_data(Dummy))
DivCal_R_df$SeqRun <- factor(DivCal_R_df$SeqRun)
DivCal_R_df$Parity <- factor(DivCal_R_df$Parity)
DivCal_R_df$motherid <- factor(DivCal_R_df$motherid)
DivCal_R_df$T1Dstatus <- factor(DivCal_R_df$T1Dstatus)
DivCal_R_df <- subset(DivCal_R_df, GBS!="NA")
#CATEGORIZE BMI FOR TEST
DivCal_R_df$BMI_Cat <- cut(DivCal_R_df$BMI, breaks = c(-Inf,18.5,25,30,Inf), labels = c("Underweight","Normal","Overweight", "Obese"))
DivCal_R_df$BMI_Cat <- as.factor(DivCal_R_df$BMI_Cat)
DivCal_R_df <- subset(DivCal_R_df, BMI_Cat!="NA")
...
##Observed richness
ob_GBS <- glmer.nb(Observed ~ T1Dstatus*GBS + Parity*GBS + BMI_Cat*GBS + (1|motherid) + (1|SeqRun), data=DivCal_R_df)
ResObs <- Anova(ob_GBS)
RGBS <- round(ResObs$`Pr(>Chisq)`[2], 2)

Dummy <- subset_samples(Dummy, GBS!="NA")
Rich_P <- plot_richness(Dummy, x="GBS", scales = "fixed", measures=c("Observed"))
Rich_P$layers <- Rich_P$layers[-1]
Rich_Plot1 <- Rich_P + geom_boxplot(data=Rich_P$data, aes(color= GBS), alpha=0.1, size=1)  +  theme(axis.title.y = element_text(size=14), axis.title.x = element_blank()) + theme(axis.text.y =element_text(size=15), axis.text.x=element_blank(), plot.title = element_text(size=15))  + theme(legend.title = element_text(size=15), legend.text = element_text(size=15)) + labs(color="GBS") + scale_fill_manual(values=GBScol) + scale_color_manual(values=GBScol) + geom_jitter() +  annotate("text", label = paste("P=", RGBS), x=1.5, y =40, size = 4, colour = "black") + theme(legend.position='none')

# InvSimpson

in_GBS <- glmer.nb(round(((DivCal_R_df$InvSimpson)*1000),0) ~ T1Dstatus*GBS + BMI_Cat*GBS + Parity*GBS + (1|motherid)  + (1|SeqRun), data=DivCal_R_df)
ResObs3 <- Anova(in_GBS)
IRGBS <- round(ResObs3$`Pr(>Chisq)`[2], 4)

Rich_P <- plot_richness(Dummy, x="GBS", scales = "fixed", measures=c("InvSimpson"))
Rich_P$layers <- Rich_P$layers[-1]
Rich_Plot2 <- Rich_P + geom_boxplot(data=Rich_P$data, aes(color= GBS), alpha=0.1, size=1)  +  theme(axis.title.y = element_text(size=11), axis.title.x = element_blank()) + theme(axis.text.y =element_text(size=15), axis.text.x=element_blank(), plot.title = element_text(size=15))  + theme(legend.title = element_text(size=15), legend.text = element_text(size=15)) + labs(color="GBS") + scale_fill_manual(values=GBScol) + scale_color_manual(values=GBScol) + geom_jitter() +  annotate("text", label = paste("P=0.0009"), x=1.5, y =5, size = 4, colour = "black") + theme(legend.position='none') + ylim(min=0, max=7.5)
cat('\n')
...
```{r BetaTestGBSImi, echo=FALSE, fig.height=20, fig.width=10, results='asis'}
Tax <- c("Species", "Genus", "Family", "Order", "Phylum")
PlotList <- vector('list', length(5))
...
OTU_Plot <- transform_sample_counts(Vec2[[i]], function(x) 100 * x/sum(x))
OTU_Plot <- transform_sample_counts(OTU_Plot, function(x) log(1 + x) )
OTU_Plot <- subset_samples(OTU_Plot, GBS!="NA")
OTU_Plot <- subset_samples(OTU_Plot, Parity!="NA")
OTU_Plot2 <- subset_samples(OTU_Plot, BMI!="NA")
...
Meta_df2$SeqRun <- as.factor(Meta_df2$SeqRun)
Meta_df2$Parity <- as.factor(Meta_df2$Parity)
Meta_df2$GBS <- as.factor(Meta_df2$GBS)
Meta_df2$BMI <- as.factor(Meta_df2$BMI)
Meta_df2$motherid <- as.factor(Meta_df2$motherid)
Meta_df2$T1Dstatus <- as.factor(Meta_df2$T1Dstatus)

#testing interaction between T1Dstatus, parity, BMI with GBS 
AdonisR <- adonis2(D2 ~  SeqRun + T1Dstatus*GBS + Parity*GBS + BMI*GBS, strata=Meta_df2$motherid,  data=Meta_df2)
print(AdonisR)
}
```
...
```{r DAGBSMicro, echo=FALSE}
for(i in 1:5){
VecOFilt <- Vec2[[i]]
VecOFilt <- prune_samples(sample_sums(VecOFilt) > 5000, VecOFilt)
VecOFilt <- subset_samples(VecOFilt, GBS!="NA")
...
Meta_df$motherid <- as.factor(as.character(Meta_df$motherid))
contrasts(Meta_df$motherid) <- contr.sum(levels(Meta_df$motherid))
Meta_df$GBS <- as.factor(Meta_df$GBS)
contrasts(Meta_df$GBS) <- contr.sum(levels(Meta_df$GBS))
Meta_df$T1Dstatus <- factor(Meta_df$T1Dstatus)
levels(Meta_df$T1Dstatus) <- c("NonT1D", "T1D") 
Meta_df$GBS_T1Dstatus <- as.factor(paste(Meta_df$GBS,Meta_df$T1Dstatus, sep="."))
contrasts(Meta_df$GBS_T1Dstatus) <- contr.sum(levels(Meta_df$GBS_T1Dstatus))

design <- model.matrix(~0 + GBS_T1Dstatus + Parity + SeqRun, data=Meta_df) #Since in comparison to the fungal data, here we have more samples, we included the factor of GBS combined with T1D status because even though the interaction was not significant in the beta diversity analysis we wanted to explore differences in GBS within levels of T1D status from the beginning.
  
dge <- DGEList(Counts, group= Meta_df$GBS_T1Dstatus)
  
dgeTMM <- edgeR::calcNormFactors(dge, method = "TMMwsp")
fit <- voomLmFit(dgeTMM, design = design , plot = FALSE, block=Meta_df$motherid)
  
## Contrast GBS_levels 
cont <- makeContrasts(GBS=(GBS_T1Dstatus0.NonT1D + GBS_T1Dstatus0.T1D)/2 - (GBS_T1Dstatus1.NonT1D + GBS_T1Dstatus1.T1D)/2, levels=design)
...
TT <- topTable(fitc, coef = "GBS", n=Inf)
TO <- TT
TV<- vector()
Mean <- cbind(data.frame(rowMeans(fit$coef[rownames(TO),1:2])), data.frame(rowMeans(fit$coef[rownames(TO),3:4])))
colnames(Mean)[1:2] <- c("MEAN:No GBS", "MEAN:GBS")
SE <- fit$stdev.unscaled * sqrt(fitc$s2.post)
SE <- cbind(data.frame(rowMeans(SE[rownames(TO),1:2])), data.frame(rowMeans(SE[rownames(TO),3:4])))
colnames(SE)[1:2] <- c("SE:No GBS", "SE:GBS")
MeanSE <- cbind(TT[,c(1,4:5)], Mean, SE)
MeanSE <- MeanSE[(MeanSE$adj.P.Val<0.1),]
  ...
  rownames(MeanSE) <- TV
    #
#For plot (Since there where only significantly DA taxa at species level, the plotting was restricted to species level)
if(i==1){
MeanSE$Taxa <- rownames(MeanSE)
MeanSE$Taxa <- sub('_', ' ', MeanSE$Taxa)
MeanSE$Taxa <- stringr::str_replace(MeanSE$Taxa, '\\*', '')
MeanSE$Taxa[3] <- c("Peptoniphilus harei")
  
X <- melt(MeanSE[,3:8], "Taxa")
Y <- data.frame(str_split_fixed(X$variable, ":", 2))
Z <- cbind(X[1], Y, X[3])
Z$X2[1:length(MeanSE$adj.P.Val)] <- rep("GBS", length(MeanSE$adj.P.Val))
Z2 <- dcast(Z, Taxa + X2 ~ X1)
Z2$adj.P.Val <- round(Z2$adj.P.Val, 3)
colnames(Z2)[2] <- "GBS status"
    
Z2$Taxa[9:10] <- c("Peptoniphilus harei", "Peptoniphilus harei")
Z2$Taxa <- as.factor(Z2$Taxa)
Z2$Taxa <- factor(Z2$Taxa, levels=MeanSE$Taxa)
Z2$`GBS status` <- relevel(Z2$`GBS status`, ref="No GBS")
Z2$Taxa[9:10] <- c("Peptoniphilus harei", "Peptoniphilus harei")
  
pd <- position_dodge(0.5)
p2 <- ggplot(Z2, aes(x=Taxa, y=MEAN, colour=`GBS status`, label=adj.P.Val))
DAMicroGBSN <-  p2 + geom_errorbar(aes(ymin=MEAN-SE, ymax=MEAN+SE), linewidth=1.5, width=.2, position=pd) + geom_point(position=pd, size=3, shape=21, fill="white") + geom_point(position=pd, size=3)  + theme(axis.title.x = element_blank()) + theme(axis.text.y =element_text(size=14), axis.text.x = element_text(angle = 45, hjust=1, size=12, face="italic")) + theme(legend.title = element_text(size=13, face="bold")) + scale_fill_manual(values=GBScol) + scale_color_manual(values=GBScol) + theme(plot.title = element_text(size=12)) + scale_y_continuous(labels = scales::number_format(accuracy = 0.1)) + geom_text(angle = 0, vjust=1, hjust=1, color="black", size=3.5, check_overlap = TRUE) + ylab("log2-transformed fitted values") + theme(legend.position='none') 
}
  ##In women with T1D
    
    TT <- topTable(fitc, coef = "GBST1D", n=Inf)
    TO <- TT
    TV<- vector()
    Mean <- data.frame(fit$coef[rownames(TO),c(2,4)])
    colnames(Mean)[1:2] <- c("MEAN:No GBS", "MEAN:GBS")
    SE <- fit$stdev.unscaled * sqrt(fitc$s2.post)
    SE <- data.frame(SE[rownames(TO),c(2,4)])
    colnames(SE)[1:2] <- c("SE:No GBS", "SE:GBS")
    MeanSE <- cbind(TT[,c(1,4:5)], Mean, SE)
    MeanSE <- MeanSE[(MeanSE$adj.P.Val<0.1),]
...
##In women without T1D

TT <- topTable(fitc, coef = "GBSNT1D", n=Inf)
TO <- TT
TV<- vector()
Mean <- data.frame(fit$coef[rownames(TO),c(1,3)])
colnames(Mean)[1:2] <- c("MEAN:No GBS", "MEAN:GBS")
SE <- fit$stdev.unscaled * sqrt(fitc$s2.post)
SE <- data.frame(SE[rownames(TO),c(1,3)])
colnames(SE)[1:2] <- c("SE:No GBS", "SE:GBS")
MeanSE <- cbind(TT[,c(1,4:5)], Mean, SE)
MeanSE <- MeanSE[(MeanSE$adj.P.Val<0.1),]
...
```{r, Count2PDmy, echo=FALSE, results='asis'}
print(kable(TB) %>% kable_styling(bootstrap_options = "striped", full_width = FALSE, position = "left"))
```
...
```{r BetaTestPDmy, echo=FALSE, fig.height=10, fig.width=10, results='asis'}
Tax <- c("Species", "Genus", "Family", "Order", "Phylum")
PlotList <- vector('list', length(5))
...
```{r AlphaTestPEmy, echo=FALSE, fig.height=5, fig.width=10, results='asis'}
sample_data(OTUVag_RA)$Any.PE <- as.factor(sample_data(OTUVag_RA)$Any.PE)
Dummy <- OTUVag_RA
DivCal_R <- estimate_richness(Dummy, measures=c("Observed", "InvSimpson"))
DivCal_R_df <- data.frame(DivCal_R, sample_data(Dummy))
...
```{r BetaTestPEmy, echo=FALSE, fig.height=15, fig.width=10, results='asis'}
PlotList <- vector('list', length(5))
...
for(i in 1:5){
Vec[[i]] <- phyloseq(otu_table(Vec[[i]], taxa_are_rows = F), sample_data(NewMet), tax_table(Vec[[i]]), phy_tree(Vec[[i]]))
Vec[[i]] <- subset_samples(Vec[[i]], GU.tract.infection!="NA")

}
for(i in 1:5){
Vec3[[i]] <- phyloseq(otu_table(Vec3[[i]], taxa_are_rows = F), sample_data(NewMet), tax_table(Vec3[[i]]), phy_tree(Vec3[[i]]))
Vec3[[i]] <- subset_samples(Vec3[[i]], GU.tract.infection!="NA")

}
for(i in 1:5){
VecP[[i]] <- phyloseq(otu_table(VecP[[i]], taxa_are_rows = F), sample_data(NewMet), tax_table(VecP[[i]]), phy_tree(VecP[[i]]))
VecP[[i]] <- subset_samples(VecP[[i]], GU.tract.infection!="NA")

}

TB <- table(sample_data(Vec[[1]])$GU.tract.infection, sample_data(Vec[[1]])$T1Dstatus)
```

For analysing the vaginal microbiome of women who had or did not have genitourinary tract infections [GUTI] there are a total of 131 samples for which we have the GUTI status. The samples are distributed between women with and without T1D as follows:

```{r, CountGUTImy, echo=FALSE, results='asis'}
print(kable(TB) %>% kable_styling(bootstrap_options = "striped", full_width = FALSE, position = "left"))
```
...
Here we will use a glm.nb only because we have no random effedct of the motherid

```{r, AlphaGUTImy, echo=FALSE}
OTUVag_RA <- Vec3[[1]]
DivCal_R <- estimate_richness(OTUVag_RA, measures=c("Observed", "InvSimpson"))
DivCal_R_df <- data.frame(DivCal_R, sample_data(OTUVag_RA))
GUTIcol <- c("deepskyblue2", "darkmagenta")
...
```{r AlphaTestGUTImy, echo=FALSE, fig.height=5, fig.width=10, results='asis'}
Dummy <- OTUVag_RA
sample_data(Dummy)$GUTI <- as.factor(sample_data(Dummy)$GU.tract.infection)
DivCal_R <- estimate_richness(Dummy, measures=c("Observed", "InvSimpson"))
DivCal_R_df <- data.frame(DivCal_R, sample_data(Dummy))
...
## Observed Richness
ob_GUTI <- glm.nb(Observed ~  T1Dstatus*GUTI + BMI_Cat*GUTI + Parity*GUTI, data=DivCal_R_df)
ResObs <- Anova(ob_GUTI)

#Using emmeans to do Contrast 
DivCal_R_df$T1DGUTI <- with(DivCal_R_df, interaction(T1Dstatus, GUTI, sep = "x"))
in_GUTI <- glm.nb(Observed ~ Parity + BMI_Cat + T1DGUTI, data=DivCal_R_df)
marginal = emmeans(in_GUTI, ~ T1DGUTI)
Contrasts = list(GUTINonT1D=c(1,0,-1,0), GUTIT1D=c(0,1,0,-1))
ResCat <- contrast(marginal, Contrasts, adjust="none")
ResIn <- data.frame(ResCat)
RGUTI_T1D <- round(ResIn$p.value[2], 3)
RGUTI_NT1D <- round(ResIn$p.value[1], 4)

##Divided by T1D status
DummyNT1D <- subset_samples(Dummy, T1Dstatus=="Non-T1D")
Rich_P <- plot_richness(DummyNT1D, x="GUTI", scales = "fixed", measures=c("Observed"))
Rich_P$layers <- Rich_P$layers[-1]
Rich_PlotNT1D <- Rich_P + geom_boxplot(data=Rich_P$data, aes(color= GUTI), alpha=0.1, size=1)  +  theme(axis.title.y = element_text(size=12), axis.title.x = element_blank()) + theme(axis.text.y =element_text(size=14), axis.text.x=element_blank(), plot.title = element_text(size=12)) +  annotate("text", label = paste("P=0.0004"), x=1.8, y =15, size = 4, colour = "black") + theme(legend.title = element_text(size=12), legend.text = element_text(size=15)) + labs(color="GUTI") + geom_jitter()  + scale_fill_manual(values=GUTIcol) + scale_color_manual(values=GUTIcol)  + theme(legend.position='none')

gridExtra::grid.arrange(Rich_PlotNT1D, Rich_PlotT1D, ncol = 2)

# InvSimpson
in_GUTI <- glm.nb(round(((DivCal_R_df$InvSimpson)*1000),0) ~ GUTI*T1Dstatus + BMI_Cat*GUTI + Parity*GUTI, data=DivCal_R_df) #Testing if the model is better when removing interactions to simplify it.
in_GUTI2 <- glm.nb(round(((DivCal_R_df$InvSimpson)*1000),0) ~ GUTI*T1Dstatus + Parity*GUTI, data=DivCal_R_df)

lrtest(in_GUTI, in_GUTI2) ## 
##Likelihood ratio test
#Model 1: round(((DivCal_R_df$InvSimpson) * 1000), 0) ~ GUTI * T1Dstatus + BMI_Cat * GUTI + Parity * GUTI
#Model 2: round(((DivCal_R_df$InvSimpson) * 1000), 0) ~ GUTI * T1Dstatus + Parity * GUTI
#Df  LogLik Df  Chisq Pr(>Chisq)
#1  12 -1127.2                     
#2   7 -1131.1 -5 7.7751     0.1691
#We can remove BMI_Cat*GUTI, so now try if we can remove Parity*GUTI

in_GUTI <- glm.nb(round(((DivCal_R_df$InvSimpson)*1000),0) ~ GUTI*T1Dstatus, data=DivCal_R_df)
lrtest(in_GUTI, in_GUTI2)
#Likelihood ratio test
#Model 1: round(((DivCal_R_df$InvSimpson) * 1000), 0) ~ GUTI * T1Dstatus
#Model 2: round(((DivCal_R_df$InvSimpson) * 1000), 0) ~ GUTI * T1Dstatus + Parity * GUTI
#Df  LogLik Df  Chisq Pr(>Chisq)
#1   5 -1133.3                     
#2   7 -1131.1  2 4.4847     0.1062
#We can remove Parity*GUTI, test if we can remove interaction with T1D

in_GUTI2 <- glm.nb(round(((DivCal_R_df$InvSimpson)*1000),0) ~ GUTI + T1Dstatus, data=DivCal_R_df)
lrtest(in_GUTI, in_GUTI2)

#Likelihood ratio test
#Model 1: round(((DivCal_R_df$InvSimpson) * 1000), 0) ~ GUTI * T1Dstatus
#Model 2: round(((DivCal_R_df$InvSimpson) * 1000), 0) ~ GUTI + T1Dstatus
#Df  LogLik Df  Chisq Pr(>Chisq)  
#1   5 -1133.3                       
#2   4 -1135.3 -1 4.0321    0.04464 *

#We can't remove the interaction of GUTI with T1Dstatus, so we stay with it.
ResIS <- Anova(in_GUTI)

#Using emmeans to make Contrasts
DivCal_R_df$T1DGUTI <- with(DivCal_R_df, interaction(T1Dstatus, GUTI, sep = "x"))
in_GUTI <- glm.nb(round(((DivCal_R_df$InvSimpson)*1000),0) ~  BMI_Cat + Parity + T1DGUTI, data=DivCal_R_df)
marginal = emmeans(in_GUTI, ~ T1DGUTI)
Contrasts = list(GUTINT1D=c(1,0,-1,0), GUTIT1D=c(0,1,0,-1))
ResCat <- contrast(marginal, Contrasts, adjust="none")
ResIn <- data.frame(ResCat)
RGUTI_NT1D <- round(ResIn$p.value[1], 2)
RGUTI_T1D <- round(ResIn$p.value[2], 2)

##Divided by T1D status
Rich_P <- plot_richness(DummyNT1D, x="GUTI", scales = "fixed", measures=c("InvSimpson"))
Rich_P$layers <- Rich_P$layers[-1]
Rich_PlotP0 <- Rich_P + geom_boxplot(data=Rich_P$data, aes(color= GUTI), alpha=0.1, size=1)  +  theme(axis.title.y = element_text(size=12), axis.title.x = element_blank()) + theme(axis.text.y =element_text(size=14), axis.text.x=element_blank(), plot.title = element_text(size=12)) +  annotate("text", label =paste("P=", RGUTI_NT1D), x=1.5, y =7, size = 4, colour = "black") + theme(legend.title = element_text(size=12), legend.text = element_text(size=15)) + labs(color="GUTI") + geom_jitter()   + scale_fill_manual(values=GUTIcol) + scale_color_manual(values=GUTIcol) + theme(legend.position='none')

GUTI_NT1D_Alpha <- gridExtra::grid.arrange(Rich_PlotNT1D, Rich_PlotP0, ncol = 2)
```

###Beta diversity

```{r BetaTestGUTImy, echo=FALSE, fig.height=20, fig.width=10, results='asis'}
Tax <- c("Species", "Genus", "Family", "Order", "Phylum")
PlotList <- vector('list', length(5))
NGUTIPlotList <- vector('list', length(5))
GUTIPlotList <- vector('list', length(5))

for(i in 1:5){
OTU_Plot <- transform_sample_counts(Vec3[[i]], function(x) 100 * x/sum(x))
OTU_Plot <- transform_sample_counts(OTU_Plot, function(x) log(1 + x) )
sample_data(OTU_Plot)$GUTI <- as.factor(sample_data(OTU_Plot)$GU.tract.infection)
OTU_Plot <- subset_samples(OTU_Plot, GUTI!="NA")
OTU_Plot2 <- subset_samples(OTU_Plot, BMI!="NA")
...
AdonisR <- adonis2(D2 ~  Parity*GUTI + BMI*GUTI + T1Dstatus*GUTI, data=Meta_df2)
print(AdonisR)
A2Res <- round(AdonisR$`Pr(>F)`[2], 2)

if(i==1 | i==5){
sample_data(OTU_Plot)$GUTI <- as.factor(sample_data(OTU_Plot)$GUTI)
ordination_OTU<-ordinate(OTU_Plot, method="CAP", distance="bray", ~  GUTI)
p_OTU12<-plot_ordination(OTU_Plot, ordination_OTU, type="samples",  color="GUTI")
PlotList[[i]] <- p_OTU12 + geom_point(size = 3)  + theme(axis.title.y = element_text(size=14), axis.title.x = element_text(size=14)) + theme(axis.text.y =element_text(size=15), axis.text.x=element_text(size=15), plot.title = element_text(size=15))  + theme(legend.title = element_text(size=15), legend.text = element_text(size=15)) +  annotate("text", label =paste("P=", A2Res), x=1.5, y =-1, size = 4, colour = "black") + ggtitle( paste(Tax[i])) + theme(legend.position='none')  + scale_fill_manual(values=GUTIcol) + scale_color_manual(values=GUTIcol) 
} else {
##Test pairwise within levels of T1D status
library("pairwiseAdonis")
#Factor to do pairwise
Meta_df2$T1DGUTI <- with(Meta_df2, interaction(T1Dstatus, GUTI, sep = "x"))
GUTIT1D <- pairwise.adonis2(D2 ~ T1DGUTI, Meta_df2, strata = NULL, nperm = 999)
GUTI_T1D <- GUTIT1D$T1Dx1_vs_T1Dx0
GUTI_NT1D <- GUTIT1D$`Non-T1Dx1_vs_Non-T1Dx0`


if(i==6){
sample_data(OTU_Plot0G)$GUTI <- as.factor(sample_data(OTU_Plot0G)$GUTI)
ordination_OTU<-ordinate(OTU_Plot0G, method="CAP", distance="bray", ~  GUTI)
p_OTU12<-plot_ordination(OTU_Plot0G, ordination_OTU, type="samples",  color="GUTI")
NGUTIPlotList[[2]] <- p_OTU12 + geom_point(size = 3)  + theme(axis.title.y = element_text(size=14), axis.title.x = element_text(size=18)) + theme(axis.text.y =element_text(size=16), axis.text.x=element_text(size=15), plot.title = element_text(size=15)) + annotate("text", label =paste("P=0.025"), x=1.5, y =-1, size = 4, colour = "black")  + theme(legend.title = element_text(size=15), legend.text = element_text(size=15)) + scale_fill_manual(values=GUTIcol) + scale_color_manual(values=GUTIcol)  + theme(legend.position='none')

sample_data(OTU_Plot0F)$GUTI <- as.factor(sample_data(OTU_Plot0F)$GUTI)
ordination_OTU<-ordinate(OTU_Plot0F, method="CAP", distance="bray", ~  GUTI)
p_OTU12<-plot_ordination(OTU_Plot0F, ordination_OTU, type="samples",  color="GUTI")
NGUTIPlotList[[3]] <- p_OTU12 + geom_point(size = 3)  + theme(axis.title.y = element_text(size=14), axis.title.x = element_text(size=18)) + theme(axis.text.y =element_text(size=16), axis.text.x=element_text(size=15), plot.title = element_text(size=15)) + annotate("text", label =paste("P=0.033"), x=1.5, y =-1, size = 4, colour = "black")  + theme(legend.title = element_text(size=15), legend.text = element_text(size=15)) + scale_fill_manual(values=GUTIcol) + scale_color_manual(values=GUTIcol)  + theme(legend.position='none')
}


####
OTU_Plot0 <- subset_samples(OTU_Plot2, T1Dstatus=="Non-T1D")
sample_data(OTU_Plot0)$GUTI <- as.factor(sample_data(OTU_Plot0)$GUTI)
ordination_OTU<-ordinate(OTU_Plot0, method="CAP", distance="bray", ~  GUTI)
p_OTU12<-plot_ordination(OTU_Plot0, ordination_OTU, type="samples",  color="GUTI")
NGUTIPlotList[[i]] <- p_OTU12 + geom_point(size = 3)  + theme(axis.title.y = element_text(size=14), axis.title.x = element_text(size=18)) + theme(axis.text.y =element_text(size=16), axis.text.x=element_text(size=15), plot.title = element_text(size=15)) + annotate("text", label =paste("P=", GUTI_NT1D$`Pr(>F)`[1]), x=1.5, y =-1, size = 4, colour = "black")  + theme(legend.title = element_text(size=15), legend.text = element_text(size=15)) + scale_fill_manual(values=GUTIcol) + scale_color_manual(values=GUTIcol)  + theme(legend.position='none')# + ggtitle( paste(Tax[i], "(Non-T1D)"))

OTU_Plot1 <- subset_samples(OTU_Plot, T1Dstatus=="T1D")  
sample_data(OTU_Plot1)$GUTI <- as.factor(sample_data(OTU_Plot1)$GUTI)
ordination_OTU<-ordinate(OTU_Plot1, method="CAP", distance="bray", ~  GUTI)
p_OTU12<-plot_ordination(OTU_Plot1, ordination_OTU, type="samples",  color="GUTI")
GUTIPlotList[[i]] <- p_OTU12 + geom_point(size = 3)  + theme(axis.title.y = element_text(size=14), axis.title.x = element_text(size=14)) + theme(axis.text.y =element_text(size=15), axis.text.x=element_text(size=15), plot.title = element_text(size=15))  + theme(legend.title = element_text(size=15), legend.text = element_text(size=15)) + scale_fill_manual(values=GUTIcol) + scale_color_manual(values=GUTIcol) + annotate("text", label =paste("P=", GUTI_T1D$`Pr(>F)`[1]), x=1.5, y =-1, size = 4, colour = "black")  + theme(legend.position='none') + ggtitle( paste(Tax[i], "(T1D)"))
}

}

gridExtra::grid.arrange(PlotList[[1]], GUTIPlotList[[2]], NGUTIPlotList[[2]], GUTIPlotList[[3]], NGUTIPlotList[[3]], GUTIPlotList[[4]], NGUTIPlotList[[4]], PlotList[[5]],  ncol = 3)
```

###Differential abundance

```{r DAGUTIMyco, echo=FALSE}
for(i in 1:5){
  VecOFilt <- Vec3[[i]]
  sample_data(VecOFilt)$GUTI <- as.factor(sample_data(VecOFilt)$GU.tract.infection)
  Counts <- as.data.frame(otu_table(VecOFilt))
  ...
  design <- model.matrix(~0 + GUTI, data=Meta_df)
  dge <- DGEList(Counts, group= Meta_df$GUTI)
  
  #Prevalence filter
  M <- dge$counts
  M[M>0] <-1
  keepP <- rowSums(M) > 13 #13 is about 10% of the samples (131) 
  dge <- dge[keepP,,keep.lib.sizes=FALSE]
  dge <- dge[,which(!dge$samples$lib.size == 0)]
  # Metadata
  dgeN <- colnames(dge$counts)
  ...
  design <- model.matrix(~0 + GUTIT1D + Parity, data=Meta_df)
  
  dgeTMM <- edgeR::calcNormFactors(dge, method = "TMMwsp")
  fit <- voomLmFit(dgeTMM, design = design , plot = FALSE)
  
  ## Contrast GUTI_levels 
  cont <- makeContrasts(GUTI=(GUTIT1D0.NonT1D + GUTIT1D0.T1D)/2 - (GUTIT1D1.NonT1D + GUTIT1D1.T1D)/2, GUTIT1D=GUTIT1D0.T1D-GUTIT1D1.T1D, GUTINT1D=GUTIT1D0.NonT1D-GUTIT1D1.NonT1D, levels=design)
  ...
  #All women with and without T1D
  TT <- topTable(fitc, coef = "GUTI", n=Inf) #No significant differences due to GUTI were found at any taxonomic level in general when all women (with and without T1D) were considered in the analysis
 
  #Only women with T1D
  TT <- topTable(fitc, coef = "GUTIT1D", n=Inf) #No significant differences due to GUTI were found at any taxonomic level in women with T1D
   
  #Only women without T1D
  TT <- topTable(fitc, coef = "GUTINT1D", n=Inf)
  TO <- TT
  TV<- vector()
  Mean <- data.frame(fit$coef[rownames(TO),c(1,3)])
...
ZA <- rbind(Zspe, Zgen, Zfam, Zord, Zphy)
ZA2 <- ZA[c(1:12,17:18,21:48),]
Orden <- unique(ZA2$Taxa)
ZA2$Taxa <- factor(ZA2$Taxa, levels=Orden)
p2 <- ggplot(ZA2, aes(x=Taxa, y=MEAN, colour=`GUTI status`, label=adj.P.Val))
DAMycGUTIAll <-  p2 + geom_errorbar(aes(ymin=MEAN-SE, ymax=MEAN+SE), size=1.5, width=.2, position=pd) + geom_point(position=pd, size=3, shape=21, fill="white") + geom_text(angle = 0, vjust=1, hjust=1, color="black", size=4, check_overlap = TRUE) + theme(axis.title.x = element_blank()) + theme(axis.text.y =element_text(size=14), axis.text.x = element_text(angle = 45, hjust=1, size=12, face="italic")) + theme(legend.title = element_text(size=13, face="bold")) + scale_fill_manual(values=GUTIcol) + scale_color_manual(values=GUTIcol)  + theme(plot.title = element_text(size=12)) + scale_y_continuous(labels = scales::number_format(accuracy = 0.1)) +  ylab("log2-transformed fitted values") + theme(legend.position='none')
##Only Species level
ZASpe <- ZA2[c(1:16),] 
p2 <- ggplot(ZASpe, aes(x=Taxa, y=MEAN, colour=`GUTI status`, label=adj.P.Val))
DAMycGUTISpe <-  p2 + geom_errorbar(aes(ymin=MEAN-SE, ymax=MEAN+SE), size=1.5, width=.2, position=pd) + geom_point(position=pd, size=3, shape=21, fill="white") + geom_text(angle = 0, vjust=1, hjust=1, color="black", size=4, check_overlap = TRUE) + theme(axis.title.x = element_blank()) + theme(axis.text.y =element_text(size=14), axis.text.x = element_text(angle = 45, hjust=1, size=12, face="italic")) + theme(legend.title = element_text(size=13, face="bold")) + scale_fill_manual(values=GUTIcol) + scale_color_manual(values=GUTIcol)  + theme(plot.title = element_text(size=12)) + scale_y_continuous(labels = scales::number_format(accuracy = 0.1)) +  ylab("log2-transformed fitted values") + theme(legend.position='none')
```

###Composed plots

```{r PlotsGUTI, echo=FALSE}
Plot1 <- ggarrange(Rich_PlotNT1D, Rich_PlotP0, NGUTIPlotList[[2]], ncol=3, labels = c("B", "C", "D")) + theme(plot.margin = margin(0.1,0.1,1,0.1, "cm") )  
F5 <- ggarrange(DAMicroGUTI, Plot1, DAMycGUTISpe, ncol=1, labels = c("A", "", "E"), heights = c(0.5, 0.35, 0.5))  + theme(plot.margin = margin(0.2,0.2,0.2,0.2, "cm") )  
```

##Group B Streptococcus infections (GBSI)

###Description of samples used in this analysis

```{r MycoGBS, echo=FALSE, fig.height=12, fig.width=16}
load(file="/$HOME/MycobiomeObjects.RData")
...
for(i in 1:5){
Vec[[i]] <- phyloseq(otu_table(Vec[[i]], taxa_are_rows = F), sample_data(NewMet), tax_table(Vec[[i]]), phy_tree(Vec[[i]]))
Vec[[i]] <- subset_samples(Vec[[i]], GBS!="NA")

}
for(i in 1:5){
Vec3[[i]] <- phyloseq(otu_table(Vec3[[i]], taxa_are_rows = F), sample_data(NewMet), tax_table(Vec3[[i]]), phy_tree(Vec3[[i]]))
Vec3[[i]] <- subset_samples(Vec3[[i]], GBS!="NA")

}
for(i in 1:5){
VecP[[i]] <- phyloseq(otu_table(VecP[[i]], taxa_are_rows = F), sample_data(NewMet), tax_table(VecP[[i]]), phy_tree(VecP[[i]]))
VecP[[i]] <- subset_samples(VecP[[i]], GBS!="NA")

}

TB <- table(sample_data(Vec[[1]])$GBS, sample_data(Vec[[1]])$T1Dstatus)
```

For analysing the vaginal microbiome of women who had or did not have Group B streptococcus [GBS] there are a total of 93 samples for which we have the GBS status. The samples are distributed between women with and without T1D as follows:

```{r, CountGBSmy, echo=FALSE, results='asis'}
print(kable(TB) %>% kable_styling(bootstrap_options = "striped", full_width = FALSE, position = "left"))
```
...
```{r AlphaTestGBSmy, echo=FALSE, fig.height=5, fig.width=10, results='asis'}
sample_data(OTUVag_RA)$GBS <- as.factor(sample_data(OTUVag_RA)$GBS)
Dummy <- OTUVag_RA
DivCal_R <- estimate_richness(Dummy, measures=c("Observed", "InvSimpson"))
DivCal_R_df <- data.frame(DivCal_R, sample_data(Dummy))
...
ob_GBS <- glm.nb(Observed ~ T1Dstatus*GBS + BMI*GBS + Parity*GBS, data=DivCal_R_df) #Removing interactions from the model (since they were not significant), does not produce a significant difference in the variable of interest, it is still significant
ResObs <- Anova(ob_GBS)
RGBS <- round(ResObs$`Pr(>Chisq)`[2], 3)

Dummy <- subset_samples(Dummy, GBS!="NA")
Rich_P <- plot_richness(Dummy, x="GBS", scales = "fixed", measures=c("Observed"))
Rich_P$layers <- Rich_P$layers[-1]
Rich_Plot1My <- Rich_P + geom_boxplot(data=Rich_P$data, aes(color= GBS), alpha=0.1, size=1)  +  theme(axis.title.y = element_text(size=11), axis.title.x = element_blank()) + theme(axis.text.y =element_text(size=15), axis.text.x=element_blank(), plot.title = element_text(size=15))  + theme(legend.title = element_text(size=15), legend.text = element_text(size=15)) + labs(color="GBS") + scale_fill_manual(values=GBScol) + scale_color_manual(values=GBScol) + geom_jitter() +  annotate("text", label = paste("P=", RGBS), x=1.5, y =20, size = 4, colour = "black") + theme(legend.position='none') 


#InvSimpson
in_GBS <- glm.nb(round(((DivCal_R_df$InvSimpson)*1000),0) ~ T1Dstatus*GBS + BMI_Cat*GBS + Parity*GBS, data=DivCal_R_df) #Removing interactions from the model (since they were not significant), does not produce a significant difference in the variable of interest, it is still NOT significant
ResObs3 <- Anova(in_GBS)
IRGBS <- round(ResObs3$`Pr(>Chisq)`[2], 3)

Rich_P <- plot_richness(Dummy, x="GBS", scales = "fixed", measures=c("InvSimpson"))
Rich_P$layers <- Rich_P$layers[-1]
Rich_Plot2My <- Rich_P + geom_boxplot(data=Rich_P$data, aes(color= GBS), alpha=0.1, size=1)  +  theme(axis.title.y = element_text(size=12), axis.title.x = element_blank()) + theme(axis.text.y =element_text(size=15), axis.text.x=element_blank(), plot.title = element_text(size=15))  + theme(legend.title = element_text(size=15), legend.text = element_text(size=15)) + labs(color="GBS") + scale_fill_manual(values=GBScol) + scale_color_manual(values=GBScol) + geom_jitter() +  annotate("text", paste("P=", IRGBS), x=1.5, y =6, size = 4, colour = "black") + theme(legend.position='none')
cat('\n')

cat('\n')
AlphaMicroGBS <- gridExtra::grid.arrange(Rich_Plot1, Rich_Plot2, ncol = 2)
```

###Beta diversity

```{r BetaTestGBSmy, echo=FALSE, fig.height=15, fig.width=10, results='asis'}
Tax <- c("Species", "Genus", "Family", "Order", "Phylum")
PlotList <- vector('list', length(5))
Nulli <- vector('list', length(2))
Multi <- vector('list', length(2))
...
#testing interaction between T1Dstatus, parity, BMI with GBS 
AdonisR <- adonis2(D2 ~  T1Dstatus*GBS + Parity*GBS + BMI*GBS,  data=Meta_df2)
print(AdonisR)
A2Res <- round(AdonisR$`Pr(>F)`[2], 2)

sample_data(OTU_Plot)$GBS <- as.factor(sample_data(OTU_Plot)$GBS)
sample_data(OTU_Plot)$Parity <- as.factor(sample_data(OTU_Plot)$Parity)
ordination_OTU<-ordinate(OTU_Plot, method="CAP", distance="bray", ~  GBS)
p_OTU12<-plot_ordination(OTU_Plot, ordination_OTU, type="samples",  color="GBS")
PlotList[[i]] <- p_OTU12 + geom_point(size = 3)  + theme(axis.title.y = element_text(size=14), axis.title.x = element_text(size=14)) + theme(axis.text.y =element_text(size=14), axis.text.x=element_text(size=14), plot.title = element_text(size=14))  + theme(legend.title = element_text(size=14), legend.text = element_text(size=14)) + scale_fill_manual(values=GBScol) + scale_color_manual(values=GBScol)  + theme(legend.position='none') +  annotate("text", label = paste("P=", A2Res), x=-3, y =-0.5, size = 4, colour = "black")# + ggtitle( paste(Tax[i])) 
cat('\n')
cat('\n')
}
gridExtra::grid.arrange(PlotList[[1]], PlotList[[2]], PlotList[[3]], PlotList[[4]], PlotList[[5]], ncol = 3)
BetaGBSMyG <- PlotList[[2]]
BetaGBSMyF <- PlotList[[3]]
```

###Differential abundance

```{r DAGBSMyco, echo=FALSE}
VecOFilt <- Vec3[[i]]
VecOFilt <- prune_samples(sample_sums(VecOFilt) > 5000, VecOFilt)
VecOFilt <- subset_samples(VecOFilt, GBS!="NA")
...
design <- model.matrix(~0 + GBS, data=Meta_df) #The general test was done with a single factor because we did not see significant interactions between GBS and T1D status in the beta diversity analysis
dge <- DGEList(Counts, group=Meta_df$GBS)

#Prevalence filter
M <- dge$counts
M[M>0] <-1
keepP <- rowSums(M) > 10 #10 based on 10% of total (87)
dge <- dge[keepP,,keep.lib.sizes=FALSE]
dge <- dge[,which(!dge$samples$lib.size == 0)]
# Metadata
dgeN <- colnames(dge$counts)
...
Counts <- as.data.frame(otu_table(VecF))
Meta_df <- as.data.frame(as.matrix(sample_data(VecF)))
design <- model.matrix(~0 + GBS, data=Meta_df)

dgeTMM <- edgeR::calcNormFactors(dge, method = "TMMwsp")
fit <- voomLmFit(dgeTMM, design = design , plot = FALSE)

## Contrast GBS_levels 
cont <- makeContrasts(GBS=GBS0 - GBS1, levels=design)
...
if(i==2){
  Zspe$Taxa <- c("Cladosporium(OTU)", "Cladosporium(OTU)", "Peniophora(OTU)", "Peniophora(OTU)")
  
  ZA <- rbind(Zspe, ZGen)
  Order <- unique(ZA$Taxa)
  Order <- Order[c(1,2,4,5,3)]
  ZA$Taxa <- factor(ZA$Taxa, levels=Order)
  ZA$`GBS status` <- relevel(ZA$`GBS status`, ref="No GBS")
  
  pd <- position_dodge(0.5)
  p2 <- ggplot(ZA, aes(x=Taxa, y=MEAN, colour=`GBS status`, label=adj.P.Val))
  DAMycoGBS <-  p2 + geom_errorbar(aes(ymin=MEAN-SE, ymax=MEAN+SE), size=1.5, width=.2, position=pd) + geom_point(position=pd, size=3, shape=21, fill="white") + theme(axis.title.x = element_blank()) + theme(axis.text.y =element_text(size=14), axis.text.x = element_text(angle = 45, hjust=1, size=12, face="italic")) + theme(legend.title = element_text(size=13, face="bold")) + scale_fill_manual(values=GBScol) + scale_color_manual(values=GBScol)   + theme(plot.title = element_text(size=12)) + scale_y_continuous(labels = scales::number_format(accuracy = 0.1)) + geom_text(angle = 0, vjust=1, hjust=1, color="black", size=3.5, check_overlap = TRUE) + ylab("log2-transformed fitted values") + theme(legend.position='none') 
}
...
Meta_df$T1Dstatus <- factor(Meta_df$T1Dstatus)
  ...
  design <- model.matrix(~0 + GBS_T1Dstatus + Parity, data=Meta_df)
  ...
  ## Contrast GBS_levels 
  cont <- makeContrasts(GBS=(GBS_T1Dstatus0.NonT1D + GBS_T1Dstatus0.T1D)- (GBS_T1Dstatus1.NonT1D + GBS_T1Dstatus1.T1D), GBSNT1D=GBS_T1Dstatus0.NonT1D-GBS_T1Dstatus1.NonT1D, GBST1D=GBS_T1Dstatus0.T1D-GBS_T1Dstatus1.T1D, levels=design)
  fitc <- contrasts.fit(fit, contrasts = cont)
  fitc <- eBayes(fitc, robust=TRUE)
  DT<- decideTests(fitc)
  summary(DT)
  ...
  if(i==3){  ##To only print the one that was significantly different
  MeanSENT1D <- MeanSE
  MeanSENT1DT1D
  }
  ...
  if(i==1){  #To only print the one that was significantly different and that was not different in the main overal test.
  MeanSET1D <- MeanSE
  MeanSET1D[1,]
  }
}
...
```{r ComposedGBS, echo=FALSE, fig.height=15, fig.width=10, results='asis'}
##GBS
plot2 <- ggarrange(Rich_Plot2, Rich_Plot1My, BetaGBSMyG, ncol=3, labels = c("B", "C", "D")) + theme(plot.margin = margin(0.2,0.2,1,0.2, "cm"))
Plot3 <- ggarrange(BetaGBSMyF, DAMycoGBS, ncol=2, labels = c("E", "F"), widths = c(0.35, 0.65))
PlotX <- ggarrange(DAMicroGBSN, plot2, Plot3, ncol=1, labels = c("A"), heights = c(0.4, 0.3, 0.3))
```
